...
export default function CreateEventStep1BasicInfoPage() {
  ...
  const updateBasicInformation = useEventCreationStore((state) => state.updateBasicInformation);

  const locationType = useEventCreationStore((state) => state.locationType);
  const setLocationType = useEventCreationStore((state) => state.setLocationType);

  const venue = useEventCreationStore((state) => state.venue);
  const onlineUrl = useEventCreationStore((state) => state.onlineUrl);
  const setOnlineUrl = useEventCreationStore((state) => state.setOnlineUrl);
  ...
  const ageRestrictions = ["All Ages (Family Friendly)", "13+", "16+", "18+", "21+"];

  const handleContinue = () => {
    if (!basicInformation.title || !basicInformation.title.trim()) {
      setValidationError("Please enter an event title before continuing.");
      return;
    }

    if (isPhysicalEvent(locationType)) {
      if (!venue || (!venue.name && !venue.address)) {
        setValidationError("Please select a physical venue for your physical event.");
        return;
      }
    } else if (isOnlineEvent(locationType)) {
      if (!onlineUrl || !onlineUrl.trim()) {
        setValidationError("Please provide a live stream or meeting link for your online event.");
        return;
      }
    } else if (isHybridEvent(locationType)) {
      if (!venue || (!venue.name && !venue.address)) {
        setValidationError("Please select a physical venue for your hybrid event.");
        return;
      }
      if (!onlineUrl || !onlineUrl.trim()) {
        setValidationError(
      ...
      <div className="p-8 max-w-4xl mx-auto w-full flex flex-col gap-6">
        ...
        <div className="bg-white border border-[#bcc9c6]/50 rounded-2xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-6 border-b border-[#bcc9c6]/30 bg-[#f9f9ff]">
            <h1 className="text-xl font-bold text-[#141b2b]">Basic Event Information</h1>
            <p className="text-xs text-[#565e74] mt-1">
              Provide essential information and set the physical or digital venue for your event.
            </p>
          </div>
          ...
          <div className="p-8 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                onChange={(e) => updateBasicInformation({ title: e.target.value })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <select
                  ...
                  onChange={(e) => updateBasicInformation({ category: e.target.value })}
              ...
              <div className="flex flex-col gap-2">
                ...
                <select
                  ...
                  onChange={(e) => updateBasicInformation({ language: e.target.value })}
                  className="w-full bg-[#f9f9ff] border border-[#bcc9c6] rounded-lg px-4 py-2.5 text-sm text-[#141b2b] focus:outline-none focus:border-[#00685f]"
                >
                  {languages.map((l) => (
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                onChange={(e) => updateBasicInformation({ ageRestriction: e.target.value })}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-semibold text-[#141b2b]">
                Event Description <span className="text-[#ba1a1a]">*</span>
              </label>
              <textarea
                rows={4}
                value={basicInformation.description}
                onChange={(e) => updateBasicInformation({ description: e.target.value })}
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="border-2 border-dashed border-[#bcc9c6] hover:border-[#00685f] bg-[#f9f9ff] rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-colors">
                <svg className="size-10 text-[#565e74] mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span className="text-xs font-semibold text-[#00685f]">Click or drag image to upload banner</span>
                <span className="text-[11px] text-[#565e74] mt-1">Recommended 16:9 ratio (PNG, JPG, max 10MB)</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-[#565e74]">
                  Choose whether attendees will join at a physical venue, online, or a combination.
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  type="button"
                  onClick={() => setLocationType(LOCATION_TYPES.PHYSICAL)}
                ...
                <button
                  type="button"
                  onClick={() => setLocationType(LOCATION_TYPES.ONLINE)}
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-semibold transition-all cursor-pointer ${
                ...
                <button
                  type="button"
                  onClick={() => setLocationType(LOCATION_TYPES.HYBRID)}
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-semibold transition-all cursor-pointer ${
              ...
              {/* Conditional Location Details UI */}
              {locationType === LOCATION_TYPES.PHYSICAL && (
                <div className="mt-2">
                ...
                <div className="bg-[#f9f9ff] border border-[#bcc9c6] rounded-xl p-6 flex flex-col gap-4 mt-2">
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-xs text-[#565e74]">
                      Provide the live stream or meeting URL where online attendees will participate.
                    </p>
                  </div>

                  <div className="flex flex-col gap-2">
                    <label className="text-[13px] font-semibold text-[#141b2b]">
                      Live Stream or Meeting Link <span className="text-[#ba1a1a]">*</span>
                    </label>
                    <input
                      type="url"
                      value={onlineUrl}
                    ...
                    <span className="text-[11px] text-[#565e74]">
                      Access link will only be shared with verified ticket holders.
                    </span>
                  </div>
                </div>
              )}
                ...
                <div className="flex flex-col gap-6 mt-2">
                  ...
                  <div className="bg-[#f9f9ff] border border-[#bcc9c6] rounded-xl p-6 flex flex-col gap-4">
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-xs text-[#565e74]">
                        Broadcast URL for attendees joining the hybrid stream remotely.
                      </p>
                    </div>
